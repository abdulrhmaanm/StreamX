...
import { fetchTrendingTv, TvShow } from "@/apis/trendingshows"
...
export default function PopularShowsOfWeek() {
  ...
  useEffect(() => {
    async function loadTrending() {
      try {
        const data = await fetchTrendingTv("week", 1)
                ...
                <div className="flex gap-4 p-3 rounded-xl border border-transparent hover:border-white/8 hover:bg-white/5 transition-all cursor-pointer group">
                  ...
                  <div className="flex flex-col justify-center gap-1.5 overflow-hidden">
                    ...
                    <p className="text-xs text-zinc-500 truncate">
                      {show.first_air_date?.slice(0, 4)} • {genres || "Unknown"}
                    </p>
                    <div className="flex items-center gap-1 text-yellow-400 text-xs">
                      <FaStar className="shrink-0" />
                      <span className="font-semibold">{show.vote_average.toFixed(1)}</span>
                      <span className="text-zinc-600">• TV Show</span>
                    </div>
                    {/* On Air badge */}
                    <span className="inline-flex w-fit items-center bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-[10px] font-bold tracking-wider uppercase px-2 py-0.5 rounded mt-0.5">
                      On Air
                    </span>
                  </div>
                </div>
